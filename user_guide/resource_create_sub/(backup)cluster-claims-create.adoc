...
spec:
  ...
  providerVsphereSpec: <6>
    ...
    vcenterFolder: vm <12>
    vcenterIp: 192.168.10.10 <13>
    vcenterKcpIp: 192.168.10.20 <14>
    vcenterMemSize: 8192 <15>
    vcenterNetwork: VM Network <16>
    vcenterResourcePool: 192.168.10.11/Resources <17>
    vcenterTemplate: ubuntu-1804-kube-v1.19.6 <18>
    vcenterThumbprint: 'A1:B2:C3:DD:E5:G6:FF:AA:AB:35:AF:17:D7:7A:38:88:55:74:A3:C2' <19>
    vmPassword: tmax@23 <20>
  version: v1.19.6 <21>
  workerNum: 1 <22>
----
+  
...
<12> 폴더 이름
<13> vCenter Server 애플리케이션(VCSA)의 IP 주소
<14> 원격 클러스터(VIP)용 컨트롤 플레인의 IP 주소
<15> VM의 메모리 크기 (단위: MB. image:../images/figure_caution_icon.png[] 단, 단위는 직접 입력하지 않는다.)
<16> 네트워크 이름
<17> 리소스 풀의 이름
<18> 클라우드 초기화의 템플릿 이름
<19> 컴퓨터 인증서의 TLS 지문
<20> VM의 root 사용자 비밀 번호
<21> Kubernetes 버전
<22> 작업자 노드 수
+
.기본 작성 예 (AWS 공급자)
