...
=== nfs 스토리지 클래스 
(QA: 현재 샘플 예시로 추가된 볼륨과 파일 스토리지 클래스는 rook-ceph의 스토리지클래스인데 hypercloud5.0에서는 ceph 사용을 권장하지 않고, nfs나 san을 주로사용하게될 예정이므로 nfs 스토리지 클래스 생성 yaml을 sample로 변경 요청드립니다.)
