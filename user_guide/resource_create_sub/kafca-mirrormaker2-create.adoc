= 카프카 미러 메이커 2 생성 (TD: 전체 검수 필요)(QA: 293381 clusters 항목 필수지정 반영하였습니다.)
:toc:
:toc-title:
...
spec:
  version: 3.0.0
  replicas: 1
  connectCluster: "my-target-cluster" (2)
  clusters: (3)
...
+
<1> 카프카 미러 메이커 2의 이름
<2> 미러링 target이 되는 클러스터 별칭 명세(별칭은 spec.clusters 목록에 있는 alias와 동일하게 지정)
<3> 미러링을 위한 카프카 클러스터의 상세 명세
