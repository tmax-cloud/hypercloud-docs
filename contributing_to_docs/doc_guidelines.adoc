...
[id="read-me"]
== AsciiDocs 문법 외 숙지할 내용 및 convention

* adoc 문법을 숙지하였다고 해도, 이 문서 전체 내용을 추가로 읽고 나서 가이드를 작성해주시기 바랍니다.

* 각 content에는 id를 부여할 수 있습니다. 필수는 아닙니다. 다른 곳에서 이 content로 링크를 달아야 할 때 유용합니다. xref:assembly-file-metadata[참고]

* adoc을 작성하면서 {product-title}, {product-version}으로 xref:../_distro_map.yml[_distro_map.yml]의 변수를 참조할 수 있습니다. +
product-title은 distro 하위의 name 속성값, product-version은 branch 하위의 name 속성값입니다. xref:product-name-version[참고]

* 여러 가지 alias를 xref:../modules/common-attributes.adoc[modules/common-attributes.adoc] 파일에 등록해두면 adoc을 작성할 때 {kebab}과 같이 사용할 수 있습니다. xref:inline-images-icons[참고]

* adoc에서 사용할 이미지들은 모두 images 폴더에 위치시킵니다. 단, 다른 문서와 공유할 가능성이 전혀 없는 이미지라면 이와 같이 하지 않아도 됩니다.

* 다른 adoc 파일의 일부를 구성할 adoc 파일은 modules 폴더에 위치시킵니다. 단, 다른 문서와 공유할 가능성이 전혀 없는 모듈이라면 이와 같이 하지 않아도 됩니다.

* 한 adoc 파일에서 image나 module을 참조하여 사용하려면, 그 부모 폴더에 images, modules 폴더로의 symbolic link가 있어야 합니다.
