...
== 브랜치 별 용도
* master : 다른 모든 브랜치에 cherry pick되어야 할 공통적인 변경사항을 우선적으로 commit하는 데 사용, 새로운 브랜치를 만들기 위한 분기점으로 사용
* 4-ko, 4-en, ... : 버전-언어 별 매뉴얼 작성에 사용, 기존 매뉴얼과 구성이나 내용이 비슷한 새 버전/언어 매뉴얼을 작성해야 할 때 브랜치 분기점으로 사용
* 기타 : 테스트를 위한 브랜치는 사용자 판단 하에 생성 (필요가 사라지면 삭제)
