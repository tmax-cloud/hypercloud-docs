...
=== 현재 브랜치에서 작업한 내용을 확인하고 테스트할 목적으로 빌드하는 경우
1. hypercloud-docs 프로젝트 폴더를 위 AsciiBinder가 설치된 환경(이하 '빌드환경')에 업로드한다. (.git 파일도 포함되어 있어야 한다.)
...
[NOTE]
====
위와 같이 하려면 모든 브랜치에 linux에서의 symbolic link가 정상적으로 생성되어 있어야 한다. +
symbolic link 문제로 이미지 참조가 안되면 linux 환경에서 각 브랜치에 symbolic link를 생성해주어야 한다. +
(각 브랜치에서 symlink 생성 + commit + push를 하거나, master 브랜치에서 이를 먼저 진행한 후 다른 브랜치에서 cherry pick)
====
