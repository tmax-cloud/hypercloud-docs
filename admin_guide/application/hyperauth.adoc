...
. *사용자 기본 정보 설정* +
*Add user* 화면에서 생성할 사용자의 기본 정보를 설정한 후 *[Save]* 버튼을 클릭한다. (TD: Realm이 Tmax일 때 사용자 이름 입력란이 없음. 확인 필요  QA: 이미지 교체 필요)
+
image::../images/figure_hyperauth_user_create_03.png[]
+
각 설정 항목에 대한 설명은 다음과 같다. (TD: 설정 항목에 대한 설명 작성 필요 여부 판단 필요  QA: 계정 생성 시 입력해야 하는 항목에 대한 설명만 아래에 추가) 
+
...
|====================
|항목|설명
|Password|사용자의 비밀번호를 입력한다.(TD: 입력 조건 작성 필요 QA: 영문 대/소문자, 특수기호, 숫자 중 3가지 이상을 조합하여 9~20자 사용하도록 권고(필수 X))
