...
|====================
|항목|설명
|Email *|로그인에 사용할 Email 주소를 입력한다.(TD: 입력 조건 작성 필요)(QA: 이메일 입력 조건 - 이메일 형식 (@ 필요), 이름&성 입력 조건 - 없음)
|First Name |이름을 입력한다.
|Last Name |성을 입력한다.
|User Enabled |ON : 로그인 허용을 허용한다.(기본값: ON)
|Email Verified |ON : 별도의 이메일 인증이 필요하다.(기본값: OFF)
