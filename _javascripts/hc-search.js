...
function hcsearch(searchParams) {
	...
	var searchprovider = `${document.location.origin}/search/json`;
	// var searchReq = { q: searchParams.q + searchParams.urlFilter, 'fields.label': searchParams.label, start: searchParams.si };
	var searchReq = { q: searchParams.q + searchParams.urlFilter, start: searchParams.si };

	hcMoreBtn.hide();
	hcSearchIndicator.show();
	$.get(searchprovider, searchReq)
		.done(function (hcsearchresults) {
			// GET success
			if (hcsearchresults == '') {
			...
			if (!$.isEmptyObject(hcsearchresults.response.result)) {
				...
				$(hcsearchresults.response.result).each(function () {
					var row = '<div class="search-result-item"><a href="' + this.url + '" target="_blank">' + this.title + '</a>';
