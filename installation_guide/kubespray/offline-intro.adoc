...
. *hypercloud23-images.tar 및 registry.tar 다운로드*
+
아래의 FTP 서버에서 hypercloud23-images.tar와 registry.tar를 다운로드한다.
+
[NOTE]
==== 
*hypercloud23-images.tar* 파일은 HyperCloud 설치에 필요한 이미지 파일이고, *registry.tar* 파일은 이미지 레지스트리를 구성하기 위한 Registry 이미지 파일이다.
====
+
----
...
. *컨테이너 실행*
+
다운로드한 hypercloud23-images.tar 파일을 압축 해제한 후 해당 이미지를 이용해서 컨테이너를 실행한다.
+
.hypercloud23-images.tar 파일 압축 해제
----
$ tar -xvf hypercloud23-images.tar
----
+
.컨테이너 실행
----
...
|{IMAGE_FILE_PATH}|hypercloud23-images.tar 파일의 압축을 해제한 경로 입력 (예: /root/hypercloud23-registry)
|====================
