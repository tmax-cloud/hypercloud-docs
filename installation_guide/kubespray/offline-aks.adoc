...
본 장에서는 AKS 환경에서 폐쇄망으로 Kubespray를 이용하여 HyperCloud를 설치하는 방법에 대해서 설명한다.
...
본 절에서는 AKS 클러스터에 연결한 Azure bastion에서 Kubespray를 이용하여 ArgoCD를 설치하는 방법에 대해서 설명한다.

ArgoCD를 설치하는 순서는 다음과 같다.

. <<KubesprayDecompressionArgoAks, Kubespray 파일 압축 해제>>
. <<KubesprayConfigArgoAks, Kubespray 환경 설정>>
. <<KubesprayRunArgoAks, Kubespray 실행>>

[#KubesprayDecompressionArgoAks]
=== Kubespray 파일 압축 해제

ArgoCD 설치를 위해 테크넷을 통해서 다운로드한 *kubespray-aks.zip* 파일의 압축을 해제한다.
...
CAUTION: RHEL 운영체제일 경우 `kubespray-aks/cluster.yml` 파일을 열어 *- { role: bootstrap-os, tags: bootstrap-os}* 행을 반드시 주석 처리해야 한다.

. *노드 정보 등록*
+ 
`kubespray-aks/inventory/tmaxcloud/inventory.ini` 파일을 열어 kubespray에서 설치할 노드들의 정보를 등록한다. +
...
. *사용자 지정 도메인 등록*
+
`kubespray-aks/inventory/tmaxcloud/group_vars/k8s_cluster/k8s-cluster.yml` 파일을 열어 외부에 노출할 사용자 지정 도메인의 정보를 등록한다.
+
.예시
----
...
해당 애플리케이션의 구성 정보는 기본적으로 `kubespray-aks/inventory/tmaxcloud/group_vars/k8s_cluster/addon.yml` 파일에서 설정이 가능하며, 추가적으로 커스터마이징이 필요할 경우에는 `kubespray-aks/roles/bootstrap-cloud/task/` 및 `kubespray-aks/roles/bootstrap-cloud/templates/` 하위 파일에서 설정이 가능하다.
+
...
.사용 방법
----
$ ansible-playbook -i ./inventory/tmaxcloud/inventory.ini ./cluster.yml -t bootstrap-cloud -e ansible_user=centos -e ansible_ssh_private_key_file={PEM_PATH} -e cloud_provider=aks -b --become-user=root
----
...
----
<1> capa 설치 여부
<2> 멀티클러스터용 AKS에서 사용하는 계정의 액세스 키 ID
