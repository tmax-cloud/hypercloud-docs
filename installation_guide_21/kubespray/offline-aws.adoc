...
.Terraform을 설치하기 위해서는 다음과 같은 사전 준비가 필요하다.(TD: 사전 방법에 대한 상세 설명이 필요한가? 필요하다면 참고 자료 요청)
[NOTE]
====
* IAM 계정 발급
* 액세스 키 ID 및 시크릿 키 저장
...
----
+
<1> (QA: 단일 마스터의 경우 마스터 노드 ip, 마스터 다중화의 경우 vip)(TD: 작성한 내용은 값 작성 방법이므로, 해당 항목의 정의 작성 필요)
<2> (QA: 단일 마스터의 경우 마스터 노드 ip, 마스터 다중화의 경우 vip)(TD: 작성한 내용은 값 작성 방법이므로, 해당 항목의 정의 작성 필요)
...
----
+
<1> (QA: calico cidr 설정) (TD: 항목 설명 필요) 
...
----
<1> ArgoCD와 연동된 Gitlab 저장소 주소 (Gitlab의 경우 url 마지막에 .git을 추가)
<2> Gitlab에 연동되어 있는 argocd-installer의 브랜치 이름 (QA: HEAD)(TD: 무엇에 대한 답변인가?)
