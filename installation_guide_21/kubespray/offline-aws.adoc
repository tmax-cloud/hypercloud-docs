...
calico_ip_auto_method: "cidr=192.168.7.0/24" <2> (QA: "cidr=20.0.6.0/24, 20.0.7.0/24, 20.0.8.0/24")
----
+
...
(QA: nfs_external_provisioner_enabled: false <4> 추가)
(QA: aws_efs_csi_enabled: true <5> 추가)
...
(QA: <4> nfs-provisioner 설치 유무)
(QA: <5> aws-efs-csi 설치 유무)
...
.사용 방법
----
$ ansible-playbook -i ./inventory/tmaxcloud/inventory.ini ./cluster.yml -e ansible_user=ec2-user -e ansible_ssh_private_key_file={PEM_PATH} -e cloud_provider=aws -b --become-user=root --flush-cache -v (QA: ansible_user=centos)
----
...
----
$ ansible-playbook -i ./inventory/tmaxcloud/inventory.ini ./cluster.yml -t bootstrap-cloud -e ansible_user=ec2-user -e ansible_ssh_private_key_file={PEM_PATH} -e cloud_provider=aws -b --become-user=root --flush-cache -v  (QA: ansible_user=centos)
----
Kubespray 실행 명령의 인자 값에 대한 설명은 다음과 같다.
