...
+
.kubespray-argocd/roles/bootstrap-cloud/defaults/main.yml (QA: `kubespray-argocd/inventory/tmaxcloud/group_vars/k8s_cluster/addon.yml` 파일에서 ingress_nginx 서비스 타입에서 바꾸는 것으로 변경해야 할 것 같습니다. "추가 설치 모듈 설정" 에 해당 내용 추가)
----
ingress_nginx_service_type: NodePort ()  (QA: ingress_nginx_service_type_pre: "LoadBalancer")
----
