...
|====================
|구분|요구사항
|운영체제|CentOS/RHEL (버전: 7, 8)(QA: 8.2, 8.4)

ProLinux (버전: 8.2, 8.3)(QA: 8.2, 8.4)
...
$ dnf config-manager --add-repo file://${FILES_REPO_PATH} (QA: ${LOCAL_REPO_PATH})

${LOCAL_REPO_PATH} 가 LOCAL_REPO_PATH에 저장된 환경변수를 불러오는거라 
export LOCAL_REPO_PATH={FILES_REPO_PATH} 단계에서 저장된 LOCAL_REPO_PATH의 환경변수값을 그대로 넣기 때문에 ${LOCAL_REPO_PATH} 로 바꿔야합니다.
인자값 설명에 LOCAL_REPO_PATH를 넣을 필요가 없지만 --add-repo files:// 주소는 ${LOCAL_REPO_PATH} 로 바꿔야 합니다. 제가 잘못 말씀드렸습니다.
----
+
