...
. (TD: all.yml 파일에 무슨 설정을 하는 것인지 키워드 작성 필요) (QA: all.yml: 쿠버네티스 기본 설정 파일)
+
...
----
+
<1> (TD: 항목 설명 필요) (QA: 단일 마스터의 경우 마스터 노드 ip, 마스터 다중화의 경우 vip)
<2> (TD: 항목 설명 필요) (QA: 단일 마스터의 경우 마스터 노드 ip, 마스터 다중화의 경우 vip)
<3> (TD: 항목 설명 필요) (QA: 쿠버네티스 api server 포트)
<4> (TD: 항목 설명 필요) (QA: 도메인네임서버 주소)
 
. (TD: k8s-net-calico.yml 파일에 무슨 설정을 하는 것인지 키워드 작성 필요)
. (QA: k8s-net-calico.yml calico 설정 파일)
+
...
----
+
<1> (TD: 항목 설명 필요) (QA: calico cidr 설정)

. (TD: addons.yml 파일에 무슨 설정을 하는 것인지 키워드 작성 필요) (QA: addons.yml 추가 설치 모듈 설정)
+
...
----
+
<1> (TD: 항목 설명 필요) 기본값으로 설정할 스토리지명
<2> (TD: 항목 설명 필요) Hyperregistry에서 postgres pvc의 storage class 명
<3> (TD: 항목 설명 필요) 그 외의 pvc storage class 명
<4> (TD: 항목 설명 필요) aws-efs-csi 스토리지 네임스페이스 명
<5> (TD: 항목 설명 필요) aws-efs filesystem id 명
...
<2> (TD: 항목 설명 필요) Hyperregistry에서 postgres pvc의 storage class 명  
<3> (TD: 항목 설명 필요) 그 외의 pvc storage class 명  
<4> (TD: 항목 설명 필요) nfs 스토리지 네임스페이스 명  
<5> (TD: 항목 설명 필요) nfs 서버 주소  
<6> (TD: 항목 설명 필요) nfs 서버 공유 폴더 위치  

. (TD: offline.yml  파일에 무슨 설정을 하는 것인지 키워드 작성 필요) (QA: offline.yml  폐쇄망 설치 시 설정 파일)
+
...
----
+
<1> (TD: 항목 설명 필요) 폐쇄망 유무
<2> (TD: 항목 설명 필요) 프라이빗 레지스트리 주소
<3> (TD: 항목 설명 필요) 파일 레포지토리 주소
...
global:
  network:
    disabled: true <2>
  domain: qa.shinhan.com <3> (QA: cloudqa.com)
  keycloak:
    domain: hyperauth.qa.shinhan.com <4> (QA: hyperauth.cloudqa.com)
...
----
...
----
<1> ArgoCD와 연동된 Gitlab 저장소 주소 (Gitlab의 경우 url 마지막에 .git을 추가)
<2> Gitlab에 연동되어 있는 argocd-installer의 브랜치 이름 (TD: (QA: HEAD)
