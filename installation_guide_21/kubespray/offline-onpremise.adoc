...
. *Kubespray 파일 압축 해제*
+ 
쿠버네티스 설치를 위해 테크넷을 통해서 다운로드한 *kubespray-infra.zip* 파일의 압축을 해제한다. (TD: 문구 검수 필요) (QA: 문구는 괜찮은데 위치가 caution 보다 위에 있어야 할 것 같습니다.)
...
. *Kubespray 파일 압축 해제*
+ 
ArgoCD 설치를 위해 테크넷을 통해서 다운로드한 *kubespray-onpremise.zip* 파일의 압축을 해제한다. (QA: 여기도 bootstrap-os 주석처리보다 압축 해제가 먼저와야 한다고 생각합니다.)
...
이때 마스터 클러스터와 싱글 클러스터에서 각각 동기화 작업을 진행해야 하며, 각 애플리케이션의 동기화 순서는 아래를 참고한다. (TD: 해당 문구 및 동기화 순서 분리 작업 내용 검수 필요)
